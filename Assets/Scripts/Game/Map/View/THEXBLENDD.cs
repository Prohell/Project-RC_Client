...
public class THEXBLENDD : MonoBehaviour
{
    /// <summary>
    /// 是否有高度
    /// </summary>
    ...
    public List<int> diagSprites = null;
    ...
    void GenHexMesh(HEX _hex)
    {
        ...
        Vector4 tangent = new Vector4(thisMatNameInd * 0.1f, neighbor4MatNameInd * 0.1f, neighbor2MatNameInd * 0.1f, neighbor3MatNameInd * 0.1f);
		...
		if (IsValid(ww, y))
		{
            if (IsValid(sw, y - 1))
			{
				...
				if ( pass == true )
                {
                    ...
                    vBld.Add(reindex_Blend(ref e, edx[3][4] - nIndexOffset, sw, y - 1, C0001, tangent));
                    vBld.Add(reindex_Blend(ref e, edx[3][5] - nIndexOffset, ww, y, C0100, tangent));  
				}
			}
		}
		...
		{   
			...
			if ( pass == true )
            {
                ...
                vBld.Add(reindex_Blend(ref e, edx[1][2] - nIndexOffset, sw, y - 1, C0001, tangent));
			}
            ...
            {
				// diag block 3
                vBld.Add(reindex_Blend(ref e, edx[1][3] - nIndexOffset, sw, y - 1, C0001, tangent));
                vBld.Add(reindex_Blend(ref e, edx[1][4] - nIndexOffset, sw, y - 1, C0001, tangent));
        ...
        if (IsValid(x, y - 1))
        {
            if (IsValid(x - 1, y - 1))
            {
                ...
                if (pass == true)
                {
                    ...
                    vBld.Add(reindex_Blend(ref e, edx[3][1] - nIndexOffset, x - 1, y - 1, C0001, tangent));
