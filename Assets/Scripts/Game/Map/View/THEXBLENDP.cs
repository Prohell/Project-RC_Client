...
public class THEXBLENDP : MonoBehaviour
{
    /// <summary>
    /// 是否有高度
    /// </summary>
    ...
    public List<int> prevSprites = null;
		...
		for (int y = 0; y < _yTile; y++)
		{
			for (int x = 0; x < _xTile; x++)
			{
				...
				if (IsValid(sw, y - 1) && !(x==0 && y==0))
				{
                    ...
                    Vector4 tangent = new Vector4(thisMatNameInd * 0.1f, -1, neighbor3MatNameInd * 0.1f, -1);
					...
					if ( pass == true && bBld)
                    {
						...
						vBld.Add( reindex_Blend(ref e, edx[1][2] - nIndexOffset, sw, y-1, C0010, tangent) );
					}
					...
					if ( pass == true && bBld)
                    {
						vBld.Add( reindex_Blend(ref e, edx[1][3] - nIndexOffset, sw, y-1, C0010, tangent) );
						vBld.Add( reindex_Blend(ref e, edx[1][4] - nIndexOffset, sw, y-1, C0010, tangent) );
                ...
                if (IsValid(ww, y))
                {
                    ...
                    Vector4 tangent = new Vector4(thisMatNameInd * 0.1f, neighbor3MatNameInd * 0.1f, neighbor4MatNameInd * 0.1f, -1);
                ...
                if (y == 0)
				{
					...
					{
                        // block 2
                        int neighbor2MatNameInd = GetMatNameInd(thisCoordX, thisCoordY, 2);
                        Vector4 tangent = new Vector4(thisMatNameInd * 0.1f, -1, neighbor2MatNameInd * 0.1f, -1);
						...
						if( pass == true )
                        {
							vBld.Add(reindex_Blend(ref e, edx[2][3] - nIndexOffset, x, y, C1000, tangent));
							vBld.Add(reindex_Blend(ref e, edx[2][4] - nIndexOffset, x, y, C1000, tangent));
							vBld.Add(reindex_Blend(ref e, edx[2][5] - nIndexOffset, x, y-1, C0010, tangent));
						}
						...
						if ( pass == true )
                        {
							vBld.Add(reindex_Blend(ref e, edx[2][0] - nIndexOffset, x, y-1, C0010, tangent));
							vBld.Add(reindex_Blend(ref e, edx[2][1] - nIndexOffset, x, y-1, C0010, tangent));
							vBld.Add(reindex_Blend(ref e, edx[2][2] - nIndexOffset, x, y, C1000, tangent));
						}
						...
						if( IsValid(x-1, y-1 ))
                        {
							...
							if ( pass == true )
                            {
								...
								{
                                    int neighbor3MatNameInd = GetMatNameInd(thisCoordX, thisCoordY, 3);
                                    tangent = new Vector4(thisMatNameInd * 0.1f, neighbor3MatNameInd * 0.1f, neighbor2MatNameInd * 0.1f, -1);

                                    vBld.Add(reindex_Blend(ref e, edx[3][0] - nIndexOffset, x, y - 1, C0010, tangent));
                                    vBld.Add(reindex_Blend(ref e, edx[3][1] - nIndexOffset, x - 1, y - 1, C0100, tangent));
                                    vBld.Add(reindex_Blend(ref e, edx[3][2] - nIndexOffset, x, y, C1000, tangent));
                                }
