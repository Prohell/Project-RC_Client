...
public class NetWorkLogic
{
    ...
    private NetWorkLogic()
    {
        ...
        m_SendbyteData = new byte[SocketOutputStream.DEFAULT_SOCKET_OUTPUT_BUFFER_SIZE]; //12K
    ...
    public void Update()
    {
        if (m_bConnectFinish)
        {
            if (null != m_delConnect) m_delConnect(m_connectStatus == ConnectStatus.CONNECTED, m_strConnectResult);
